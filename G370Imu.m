...
%
% Epson G370 IMU derived from Epson Device Uart handle class
% NOTE: Only G370PDF1 & G370PDS0 are supported
%       G370PDF0 is partially supported excluding external trigger function

classdef G370Imu < EpsonDeviceUart
    ...
    properties
        ...
        Gpio2Sel (1,:) char {mustBeMember(Gpio2Sel,{... % GPIO2 pin function selection NOTE: external trigger assumes external trigger modeB
            'gpio', 'ext_trigger', 'counter_reset'...
             })} = 'gpio';
        isPDS0 (1,1) {mustBeInteger, ... % 0=G370PDF1 (default), 1=G370PDS0
            mustBeNonnegative, mustBeLessThan(isPDS0, 2)} = 0;
    end

    properties(GetAccess = 'public', SetAccess = 'private', Hidden=true)
        % Scalefactors
        SF_GYR = 1/66; % Scale factor to convert to deg/sec
        SF_ACC = 1/2.5; % Scale factor to convert to mG (mill-gravity)
        SF_ANG = 0.00699411; % Scale factor to convert to degrees
        SF_DLTA = 3.103e-2; % Scale factor to convert to deg/bit
        SF_DLTV = 8.034e-3; % Scale factor to convert to (m/s)/bit
        SF_TEMPC = -0.0037918; % Scale factor to convert to degC
        TEMPC_OFFSET = -2634; % Offset for conversion to degC
        % Flag indicating 2000, 400, or 80sps (affects Filter Setting)
        Is2000_400_80sps = false;
    end
    ...
    properties(Constant, Hidden=true)
        ...
        RT_DIAG1 = [0, hex2dec('2A')];
        RT_DIAG2 = [0, hex2dec('2B')];
        ...
        R_MATRIX_G_M11 = [1, hex2dec('38')];
        R_MATRIX_G_M12 = [1, hex2dec('3A')];
        R_MATRIX_G_M13 = [1, hex2dec('3C')];
        R_MATRIX_G_M21 = [1, hex2dec('3E')];
        R_MATRIX_G_M22 = [1, hex2dec('40')];
        R_MATRIX_G_M23 = [1, hex2dec('42')];
        R_MATRIX_G_M31 = [1, hex2dec('44')];
        R_MATRIX_G_M32 = [1, hex2dec('46')];
        R_MATRIX_G_M33 = [1, hex2dec('48')];
        R_MATRIX_A_M11 = [1, hex2dec('4A')];
        R_MATRIX_A_M12 = [1, hex2dec('4C')];
        R_MATRIX_A_M13 = [1, hex2dec('4E')];
        R_MATRIX_A_M21 = [1, hex2dec('50')];
        R_MATRIX_A_M22 = [1, hex2dec('52')];
        R_MATRIX_A_M23 = [1, hex2dec('54')];
        R_MATRIX_A_M31 = [1, hex2dec('56')];
        R_MATRIX_A_M32 = [1, hex2dec('58')];
        R_MATRIX_A_M33 = [1, hex2dec('5A')];

        % Device Timings
        SELFTEST_DELAY = 150e-3;
        FLASHTEST_DELAY = 5e-3;
    end

    methods

        function obj = G370Imu(comport, baudrate, isPDS0)
        % class constructor
        ...
        % Property isPDS0 sets the proper SF_GYR (160deg/s vs 450deg/s)
        % Will automatically enter CONFIG mode
            ...
            if strcmpi(obj.Gpio2Sel, 'external_trigger')
                if (logical(obj.DeltaA) || logical(obj.DeltaV))
                    fprintf('Error: DeltaAngle-DeltaVelocity cannot be enabled ');
                    fprintf('when external_trigger is enabled.');
                    fprintf('Disabling DeltaAngle-DeltaVelocity as fallback.');
                    obj.DeltaA = 0;
                    obj.DeltaV = 0;
                end
            end
            ...
            obj.setAttiCtrl();
            obj.setUartCtrl();
            if logical(obj.isPDS0)
               obj.SF_GYR = 1/150; % PDS0 Scale factor to convert to deg/s
            end
        end
        ...
        % Read back all register values
            ...
            obj.readReg(obj.RT_DIAG1, true);
            obj.readReg(obj.RT_DIAG2, true);
            ...
            obj.readReg(obj.R_MATRIX_G_M11, true);
            obj.readReg(obj.R_MATRIX_G_M12, true);
            obj.readReg(obj.R_MATRIX_G_M13, true);
            obj.readReg(obj.R_MATRIX_G_M21, true);
            obj.readReg(obj.R_MATRIX_G_M22, true);
            obj.readReg(obj.R_MATRIX_G_M23, true);
            obj.readReg(obj.R_MATRIX_G_M31, true);
            obj.readReg(obj.R_MATRIX_G_M32, true);
            obj.readReg(obj.R_MATRIX_G_M33, true);
            obj.readReg(obj.R_MATRIX_A_M11, true);
            obj.readReg(obj.R_MATRIX_A_M12, true);
            obj.readReg(obj.R_MATRIX_A_M13, true);
            obj.readReg(obj.R_MATRIX_A_M21, true);
            obj.readReg(obj.R_MATRIX_A_M22, true);
            obj.readReg(obj.R_MATRIX_A_M23, true);
            obj.readReg(obj.R_MATRIX_A_M31, true);
            obj.readReg(obj.R_MATRIX_A_M32, true);
            obj.readReg(obj.R_MATRIX_A_M33, true);
    ...
    % this class).
        ...
        % NOTE: For G370, FilterSel differs when DOUT_RATE=2000, 400, 80sps
            switch (filterSel)
                case 0
                    obj.FilterSel = 'tap0';
                case 1
                ...
                case 8
                    if obj.Is2000_400_80sps
                        obj.FilterSel = '32fc50';
                    else
                        obj.FilterSel = '32fc25';
                    end
                case 9
                    if obj.Is2000_400_80sps
        ...
        % Update Is2000_400_80sps flag as necessary
            switch(obj.DoutRate)
                case 2000
                    wbyte = 0;
                case 1000
            ...
            % Update the Is2000_400_80sps flag
            if obj.DoutRate == 2000 || ...
                    obj.DoutRate == 400 || ...
                    obj.DoutRate == 80
                obj.Is2000_400_80sps = true;
            else
                obj.Is2000_400_80sps = false;
            end
        end
            ...
            if obj.Is2000_400_80sps
                ...
                switch(obj.FilterSel)
                    ...
                    otherwise
                        ...
                        fprintf('Must be tap0, tap2, tap4,\n');
                        fprintf('tap8, tap16, tap32, tap64, tap128,\n');
                        fprintf('32fc25, 32fc50, 32fc100, 32fc200,\n');
                        fprintf('64fc25, 64fc50, 64fc100, 64fc200,\n');
                        fprintf('128fc25, 128fc50, 128fc100, 128fc200,\n');
                        fprintf('Defaulting to tap16\n');
                        wbyte = hex2dec('4');
                        obj.FilterSel = 'tap16';
                end
        ...
        % Set ATTI_CTRL based on object properties
            ...
            if bitor(logical(obj.DeltaA), logical(obj.DeltaV))
