...
%
% Epson G365 IMU derived from Epson Device Uart handle class
% NOTE: Only G365PDF1 & G365PDC1 are supported
%       G365PDF0 is partially supported excluding external trigger function

classdef G365Imu < EpsonDeviceUart
    ...
    properties
        ...
        IsPDC1 (1,1) {mustBeInteger, ... % 0=G365PDF1 (default), 1=G365PDC1
            mustBeNonnegative, mustBeLessThan(IsPDC1, 2)} = 0;
    end

    properties(GetAccess = 'public', SetAccess = 'private', Hidden=true)
        % Scalefactors (initially default to G365PDF1)
    ...
    methods
        ...
        function obj = G365Imu(comport, baudrate, IsPDC1)
        % class constructor
            ...
            obj.getDeviceCfg();
            if not(strcmpi(obj.ProdId(1:4), 'G365'))
        ...
        % Property IsPDC1 sets the proper SF_ACC (10G vs 4G)
        % Will automatically enter CONFIG mode
            ...
            obj.setAttiCtrl();
            obj.setUartCtrl();
            if logical(obj.IsPDC1)
               obj.SF_ACC = 1/6.25; % PDC1 Scale factor to convert to mG (milli-gravity)
               obj.SF_DLTV = 3.213e-3; % PDC1 Scale factor to convert to m/s
            else
               obj.SF_ACC = 1/2.5; % PDF1 Scale factor to convert to mG (milli-gravity)
               obj.SF_DLTV = 8.034e-3; % PDF1 Scale factor to convert to m/s
            end
        end
