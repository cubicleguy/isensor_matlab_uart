...
%
% Epson G370G, G370T IMU derived from Epson Device Uart handle class
% NOTE: For G370PDF1 & G370PDS0 support refer to G370F_G370SImu.m

classdef G370G_G370TImu < EpsonDeviceUart
    ...
    properties
        ...
        Gpio2Sel (1,:) char {mustBeMember(Gpio2Sel,{... % GPIO2 pin function selection
            'gpio', 'ext_trigger', 'counter_reset'...
             })} = 'gpio';
        IsPDT0 (1,1) {mustBeInteger, ... % 0=G370PDG0 (default), 1=G370PDT0
            mustBeNonnegative, mustBeLessThan(IsPDT0, 2)} = 0;
    ...
    methods
        ...
        function obj = G370G_G370TImu(comport, baudrate, IsPDT0)
        % class constructor
            ...
            obj.getDeviceCfg();
            if not(or(strcmpi(obj.ProdId, 'G370PDG0'), strcmpi(obj.ProdId, 'G370PDT0')))
        ...
        % Property IsPDT0 sets the proper SF_GYR (160deg/s vs 450deg/s)
        ...
        % Will automatically enter CONFIG mode
            ...
            obj.setAttiCtrl();
            obj.setUartCtrl();
            if logical(obj.IsPDT0)
               obj.SF_GYR = 1/150; % PDT0 Scale factor to convert to deg/s
            else
               obj.SF_GYR = 1/66; % PDG0 Scale factor to convert to deg/s
            end
